...
// static constexpr gpio_num_t MOTOR01_PIN = GPIO_NUM_12;
static constexpr auto MOTOR01_PIN = 12;
static constexpr auto MOTOR02_PIN = 2;
...
static constexpr auto MOTOR02_MAGNET_COUNT = 14;
...
void loop()
{
    ...
    if (continuous_throttle && (millis() - last_stats_print >= 5000))
    {
        ...
        motor02.printDShotInfo();

        // Get Motor RPM if bidirectional
        if (IS_BIDIRECTIONAL)
        {
            dshot_telemetry_result_t telem_result01 = motor01.getTelemetry(MOTOR01_MAGNET_COUNT);
            dshot_telemetry_result_t telem_result02 = motor02.getTelemetry(MOTOR01_MAGNET_COUNT);
            printTelemetryResult(telem_result01);
            printTelemetryResult(telem_result02);
        }
...
void handleSerialInput(const String &input, uint16_t &throttle, bool &continuous_throttle)
{
    ...
    else if (input.startsWith("cmd "))
    {
        ...
        {
            dshot_result_t result01 = motor01.sendCommand(cmd_num);
            dshot_result_t result02 = motor02.sendCommand(cmd_num);
            printCommandResult(result01, "DShot Command " + String(cmd_num));
        }
        else
        {
